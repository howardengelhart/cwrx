...
describe('content category endpoints (E2E):', function() {
    ...
    describe('GET /api/content/categories', function() {
        ...
        xit('should return a 200 and empty array if nothing is found', function(done) {
            var options = {
                url: config.contentUrl + '/content/categories?user=hamboneHarry',
                jar: e2eUserJar
            };
            requestUtils.qRequest('get', options).then(function(resp) {
                expect(resp.response.statusCode).toBe(200);
        ...
        xit('should be able to sort and paginate the results', function(done) {
            var options = {
                jar: e2eUserJar,
                url: config.contentUrl + '/content/categories?ids=e2e-getquery1,e2e-getquery2,e2e-getquery3' +
                                         '&limit=2&sort=id,-1'
            };
            requestUtils.qRequest('get', options).then(function(resp) {
                expect(resp.response.statusCode).toBe(200);
        ...
        xit('should prevent mongo query selector injection attacks', function(done) {
            var options = {
                url: config.contentUrl + '/content/categories?user[$gt]=',
                jar: e2eUserJar
            };
            requestUtils.qRequest('get', options).then(function(resp) {
                expect(resp.response.statusCode).toBe(200);
        ...
        xit('should throw a 401 error if the user is not authenticated', function(done) {
            var options = {
                url: config.contentUrl + '/content/categories?user=e2e-user'
            };
            requestUtils.qRequest('get', options).then(function(resp) {
                expect(resp.response.statusCode).toBe(401);
                expect(resp.body).toBe('Unauthorized');
