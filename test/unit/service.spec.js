...
describe('service (UT)',function(){
    ...
    describe('loginServiceUser', function() {
        var resp;
        ...
        beforeEach(function() {
            ...
            spyOn(requestUtils, 'qRequest').and.returnValue(q(resp));
            state.secrets = {
                sixxyCredentials: {
                    email: 'sixxy',
                    password: 'password'
        ...
        describe('when the login is a success', function() {
            beforeEach(function() {
                resp.response.statusCode = 200;
            });

            it('should login the sixxy user', function(done) {
                var expectedOptions = {
                ...
                service.loginServiceUser(state).then(function() {
                    expect(requestUtils.qRequest).toHaveBeenCalledWith('post', expectedOptions);
                }).catch(function(error) {
                    expect(error).not.toBeDefined();
                }).done(done);
            });

            describe('if the c6Auth cookie is returned', function() {
                beforeEach(function() {
                    resp.response.headers['set-cookie'] = ['c6Auth=cookie'];
                });

                it('should set the cookie on the state', function(done) {
                    service.loginServiceUser(state).then(function() {
                        expect(state.secrets.sixxyCookie).toBe('c6Auth=cookie');
                    }).catch(function(error) {
                        expect(error).not.toBeDefined();
                    }).done(done);
                });
            ...
            describe('if the c6Auth cookie is not returned', function() {
                beforeEach(function() {
                    resp.response.headers['set-cookie'] = ['something=cookie'];
                });

                it('should reject the promise', function(done) {
                    service.loginServiceUser(state).then(function(state) {
                        expect(state).not.toBeDefined();
                    }).catch(function(error) {
                        expect(error).toBe('Failed to login user for the service: \'c6Auth cookie not found\'');
                    }).done(done);
                });
            });
        });

        describe('when the login was a failure', function() {
            beforeEach(function() {
                resp.response.statusCode = 500;
                resp.body = 'login error';
            });
            ...
            it('should reject the promise', function(done) {
                service.loginServiceUser(state).then(function(state) {
                    ...
                    expect(error).toBe('Failed to login user for the service: \'login error\'');
                }).done(done);
            });
        });
    });
