(function(){
    ...
    service.loginServiceUser = function(state) {
        var log = logger.getLog();
        var api = state.config.api;
        var sixxyCreds = state.secrets.sixxyCredentials;
        var opts = {
            url: url.resolve(api.root, api.auth.endpoint + '/login'),
            json: {
                email: sixxyCreds.email,
                password: sixxyCreds.password
            }
        };
            ...
            .then(function(resp) {
                if(resp.response.statusCode === 200) {
                    ...
                    for(var i=0;i<cookies.length;i++) {
                        if(cookies[i].substring(0, 7) === 'c6Auth=') {
                            state.secrets.sixxyCookie = cookies[i];
                            log.info('Successfully logged in service user %1', resp.body.id);
                            return q(state);
                        }
                    }
                    return q.reject('c6Auth cookie not found');
                } else {
                    return q.reject(resp.body);
                }
            })
            .catch(function(error) {
                return q.reject('Failed to login user for the service: ' + util.inspect(error));
            });
    };
