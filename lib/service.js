(function(){
    ...
    service.initMongo = function(state) {
        ...
        return q.all(Object.keys(state.config.mongo).map(function(dbName) {
            ...
            function doConnect(){
                mongoUtils.connect(host, port, dbName, username, password)
                .then(function(db) {
                    log.info('Successfully connected to %1 at %2:%3', dbName, host, port);
                    state.dbs[dbName] = db;
                    return true;
                }).then(function() {
    ...
    service.initSessionStore = function(state) {
        ...
        state.sessionStore = new MongoStore({
            ...
            log.info('Successfully connected to sessions at %1:%2', host, port);
            deferred.resolve(state);
        });
