(function(){
    ...
    service.initPubSubs = function(state) { //TODO: rename? comment, test
        var log = logger.getLog();
        
        state.publishers = {}; //TODO: unify? rename? should these go under .pubsub?
        state.subscribers = {};
        ...
        return q.all(Object.keys(state.config.pubsub).map(function(channel) {
            ...
            if (state.config.pubsub[channel].isPublisher) {
                ...
                state.subscribers[channel] = new pubsub.Subscriber(
                    channel,
                    connCfg,
                    state.config.pubsub[channel].pollDelay || undefined //TODO: weird to have this at same level as potential publisher opts?
                );
            }
        }))
        .thenResolve(state);
    ...
    service.initCache = function(state) {
        ...
        if (!state.config.cache || !state.config.cache.servers) {
            log.info('No cache config, not initializing the cache');
            return q(state);
        }
        
        log.info('Connecting to cache at: %1', state.config.cache.servers);
        return cacheLib.createCache(
