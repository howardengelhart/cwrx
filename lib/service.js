    ...
    function doConnect(){
        ...
        .then(function(db) {
            ...
            if (state.config.sessions && state.config.sessions.db) {
                return mongoUtils.connect(host, port, state.config.sessions.db, username, password)
                .then(function(sessionsDb) {
                    state.sessionsDb = sessionsDb;
                    log.info('Successfully connected and authenticated to sessionsDb %1',
