(function(){
    ...
    service.initMongo = function(state) {
        var log = logger.getLog(), host, port, username, password, deferred;
        ...
        function doConnect(){
            mongoUtils.connect(host, port, state.config.mongo.db, username, password)
            .then(function(db) {
                log.info('Successfully connected and authenticated to mongo at %1:%2', host, port);
                state.db = db;
                return true;
            }).then(function() {
                deferred.resolve(state);
            }).catch(function(error) {
                if (state.config.mongo.retryConnect && error.errmsg !== 'auth fails') {
    ...
    service.initSessionStore = function(state) {
        if (!state.config.mongo || !state.config.sessions || !state.secrets ||
            !state.secrets.mongoCredentials) {
            return q.reject({
                message: 'Need mongo config, sessions config, and secrets with credentials'
            });
        }
        ...
        state.sessionStore = new MongoStore({
            ...
            log.info('Successfully connected and authenticated to sessions db %1',
                     state.config.sessions.db);
            deferred.resolve(state);
        });

        return deferred.promise.then(function(state) {
            state.sessionStore.db.bufferMaxEntries = 0;
            return state;
        });
    };
