(function(){
    ...
    service.configure = function(state){
        ...
        if (state.config.rcAppCredsPath && fs.existsSync(state.config.rcAppCredsPath)) {
            state.rcAppCreds = fs.readJsonSync(state.config.rcAppCredsPath);
        }
    ...
    service.cluster = function(state){
        ...
        cluster.on('exit', function(worker, code, signal) {
            ...
            if (worker.suicide === true){
                log.info('Worker %1 died peacefully', worker.process.pid);
            } else {
                log.error('Worker %1 died, code %2, signal %3, restarting...',
    ...
    service.initSessions = function(state) {
        var mongoUtils  = require('./mongoUtils'),
            sessionLib  = require('express-session'),
            MongoStore  = require('connect-mongo')(sessionLib),
            log         = logger.getLog();
        if (state.clusterMaster) {
        ...
        var host        = state.config.sessions.mongo.host,
        ...
        function doConnect() {
            ...
            .then(function(db) {
                ...
                state.sessionStore = new MongoStore({
                    stringify: false,
                    touchAfter: state.config.sessions.maxAge, // ensures cookie TTLs not extended
                    db: db
                });

                state.sessions = sessionLib({
        ...
        process.nextTick(doConnect);
        return deferred.promise.thenResolve(state);
    };

    /* Initialize pub/sub channels based on config. Subscriber clients are stored in
     * state.subscribers and Publisher clients are stored in state.publishers */
    service.initPubSubChannels = function(state) {
        ...
        return q.all(Object.keys(state.config.pubsub).map(function(channel) {
            ...
            if (state.config.pubsub[channel].isPublisher) {
                ...
                state.subscribers[channel] = new pubsub.Subscriber(
                    channel,
                    connCfg,
                    state.config.pubsub[channel].opts
                );
            }
        }))
        .thenResolve(state);
    ...
    service.initCache = function(state) {
        var cacheLib    = require('./cacheLib'),
            log         = logger.getLog(),
            servers, cfgChannel;

        if (!state.config.cache) {
            log.info('No cache config, not initializing the cache');
            return q(state);
        }
    ...
    // Initialize the connection to postgres, setting necessary default settings
    service.initPostgres = function(state) {
