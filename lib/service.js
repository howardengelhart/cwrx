            ...
            function doConnect(){
                ...
                .then(function(db) {
                    ...
                    if (state.config.mongo[dbName].retryConnect && error.errmsg !== 'auth fails') {
                        log.trace('retrying connect to %1', dbName);
                        setTimeout(doConnect, 1000);
                        log.error(error);
                        deferred.notify('retry');
                    } else {
                        log.trace('not retrying connect to %1', dbName);
        ...
        function doConnect(){
            ...
            .then(function(db) {
                ...
                if (state.config.sessions.mongo.retryConnect && error.errmsg !== 'auth fails') {
                    log.trace('retrying connect to sessions');
                    setTimeout(doConnect, 1000);
                    log.error(error);
                    deferred.notify('retry');
                } else {
                    log.trace('not retrying connect to sessions');
                    deferred.reject(error);
                }
            });
        }
