...
service.initMongo = function(state) {
    ...
    var username = state.secrets.mongoCredentials.user;
    var password = state.secrets.mongoCredentials.password;
    return mongoUtils.connect(state.config.mongo.host, state.config.mongo.port)
    .then(function(mongoClient) {
        ...
        return q.npost(state.db, 'authenticate', [username, password]);
    }).then(function() {
