            ...
            function doConnect(){
                ...
                .then(function(db) {
                    ...
                    db.on('error', function(error) {
                        ...
                        log.info('Exiting app to try a restart');
                        // process.exit(1);
                        state.dbStatus[dbName] = 'down';
                        state.dbs[dbName] = null;
                        process.nextTick(doConnect);
                    });
                    db.once('close', function() {
                        log.error('Connection to %1 closed, exiting to try a restart', dbName);
                        // process.exit(1);
                        state.dbStatus[dbName] = 'down';
                        state.dbs[dbName] = null;
                        process.nextTick(doConnect);
                    ...
                    state.dbs[dbName] = db;
                    state.dbStatus[dbName] = state.dbStatus[dbName] === 'down' ? 'recovered' : 'ok';
                    return true;
                }).then(function() {
        ...
        function doConnect(){
            ...
            .then(function(db) {
                ...
                db.on('error', function(error) {
                    ...
                    log.info('Exiting app to try a restart');
                    // process.exit(1);
                    state.dbStatus.sessions = 'down';
                    state.sessionStore = null;
                    process.nextTick(doConnect);
                });
                db.on('close', function() {
                    log.error('Connection to sessions closed, exiting to try a restart');
                    // process.exit(1);
                    state.dbStatus.sessions = 'down';
                    // state.sessionStore = null;
                ...
                state.dbStatus.sessions = state.dbStatus.sessions === 'down' ? 'recovered' : 'ok';
                return true;
            }).then(function() {
                deferred.resolve(state);
            }).catch(function(error) {
