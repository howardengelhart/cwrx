    ...
    //    console.log('strlen: ' + strlen);
        for ( ;i < strlen; ) {
            ...
            for (; rawStr.charCodeAt(i) !== opts.dInner; ) {
                if (++i >= strlen) {
                    throw new Error('inner delim[' + opts.dInner +
                        '] not found at [' + i + '] [index=' + index + ']');
                }
            }
            var tag = rawStr.substring(tokenStart,i);
            tokenStart = ++i;

            for (; rawStr.charCodeAt(i) !== opts.dOuter; ) {
                if (++i >= strlen)  {
                    if (opts.requireClosingDelim) {
                        throw new Error('outer delim[' + opts.dOuter +
                            '] not found at [' + i + '] [index=' + index + ']');
                    } else {
                        break;
                    }
            ...
            var val = rawStr.substring(tokenStart,i);
