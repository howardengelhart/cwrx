            ...
            return promise.then(function() {
                ...
                .then(function(resp) {
                    log.info('Created banner "%1", id %2, for campaign %3',
                             resp.name, resp.id, campId);

                    newBanns.forEach(function(item) {
                        if (item.id === resp.extId) {
                            if (type === 'card') {
                                item.campaign.bannerNumber = resp.bannerNumber;
                                item.campaign.bannerId = parseInt(resp.id);
                            } else {
    ...
    bannerUtils.cleanBanners = function(newBanns, oldBanns, campId) {
        ...
        return oldBanns.reduce(function(promise, oldBann) {
            ...
            return promise.then(function() {
                ...
                .catch(function(error) {
                    try {
                        var rgx = /cannot remove the last.*banner from.*campaign/;
                        if (rgx.test(error.root.Envelope.Body.Fault.faultstring)) {
                            log.info('Cannot delete banner %1, it\'s the last in active camp %2',
                                     oldBann.bannerId, campId);
                            
                            // calling code should handle this + return 400, not reject
                            return q.reject({
                                c6warn: 'Cannot delete last active banner in campaign'
                            });
                        }
                    } catch(e) {}
