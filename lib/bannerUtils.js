(function(){
    ...
    // Format a banner for a Click Command campaign
    bannerUtils.formatClickBanner = function(obj) {
        var retObj = {};

        retObj.banner = {
            activeFilename      : 'http://imageserv.adtech.de/images/AdTracImage.gif',
            description         : obj.description,
            extId               : obj.id,
            fileType            : 'html', // this feels wrong
            id                  : -1,
            linkUrl             : obj.targetLink,
            mainFileName        : 'index.html', // so so wrong
            name                : 'Click Command',
            redirectUrl         : 'http://imageserv.adtech.de/images/AdTracImage.gif',
            sizeTypeId          : 16,
            statusId            : kBanner.STATUS_ACTIVE,
            styleTypeId         : kBanner.STYLE_REDIRECT
        };
        retObj.bannerInfo = {
    ...
    bannerUtils.createBanners = function(newBanns, oldBanns, type, isSponsored, campId) {
        ...
        newBanns.forEach(function(item, idx) {
            ...
            if (!existing) { // Create new banners for anything not in oldBanns
                if (type === 'clickCommand') {
                    toCreate.push(bannerUtils.formatClickBanner(item));
                } else {
