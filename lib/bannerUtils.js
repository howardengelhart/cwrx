(function(){
    ...
    bannerUtils.formatBanner = function(type, extId) {
        var retObj = {},
    ...
    bannerUtils.createBanners = function(bannCfgs, oldBanns, type, campId) {
        ...
        bannCfgs.forEach(function(item, idx) {
            ...
            if (!existing) { // Create new banners for anything not in oldBanns
                toCreate.push(bannerUtils.formatBanner(type, item.id));
    ...
    bannerUtils.cleanBanners = function(newBanners, oldBanners, campId) {
        var log = logger.getLog();
