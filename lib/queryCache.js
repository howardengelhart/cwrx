(function(){
    ...
    QueryCache.prototype.getPromise = function(query, sort, limit, skip) {
        var self = this,
            ...
            key = hashUtils.hashText(JSON.stringify(keyObj)).substr(0,18),
            deferred = self._keeper.getDeferred(key, true);

        if (deferred) {
            var age = new Date() - deferred.keeperCreateTime;
            if (age > self.maxTTL) {
                log.info('Query %1 too old, removing', JSON.stringify(keyObj));
                self._keeper.remove(key, true);
            } else if (age > self.freshTTL && !deferred.refreshStarted) {
                deferred.refreshStarted = true;
                log.info('Query %1 needs to be refreshed', JSON.stringify(keyObj));
                process.nextTick(function() {
                    q(self._coll.find(query, { sort: sort, limit: limit, skip: skip }).toArray())
                    .then(function(results) {
                        log.trace('Query %1 refreshed successfully', JSON.stringify(keyObj));
                ...
                log.info('Query %1 cache hit', JSON.stringify(keyObj));
                return deferred.promise;
            }
        }
        log.info('Query %1 cache miss', JSON.stringify(keyObj));
