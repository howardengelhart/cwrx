(function(){
    ...
    pubsub.Publisher = function(name, connCfg) {
        ...
        self.name = name;
        self._sockets = [];
        self._lastClientId = 0; // increments on client connect, gives sockets helpful ids
        self.lastMsg = '';
        
        self._server = net.createServer();
        
        self._server.on('connection', function(sock) {
            sock._id = ++self._lastClientId;
            log.info('Publisher %1 got new client #%2', self.name, sock._id);
            self._sockets.push(sock);
            
            log.trace('Publisher %1 sending client #%2 last message: %3',
                      self.name, sock._id, self.lastMsg);
            sock.write(self.lastMsg);
            
            sock.on('end', function() {
                log.info('Client #%1 disconnected from publisher %2', sock._id, self.name);
                self._sockets = self._sockets.filter(function(s) {
                    return s._id !== sock._id;
                });
            })
            .on('error', function(err) {
                log.warn('Conn to client #%1 for publisher %2 got error: %3',
                         sock._id, self.name, util.inspect(err));
                sock.end();
                self._sockets = self._sockets.filter(function(s) {
                    return s._id !== sock._id;
                });
    ...
    pubsub.Subscriber = function(name, connCfg, opts) {
        ...
        .on('error', function(err) {
            var msg = 'Subscriber %1 got error, disconnecting: %2';
            if (!self.reconnect._interval) {
                log.warn(msg, self.name, util.inspect(err));
            } else { // don't spam warnings when reconnect attempts fail
                log.info(msg, self.name, util.inspect(err));
            }
            self.beginReconnect();
        });
    };
