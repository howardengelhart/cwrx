...
(function(){
    ...
    Cache.prototype._memcachedCommand = function(cmd, opType, args) {
        var log = logger.getLog(),
            self = this,
            keyStr = args && args[0] && ('on ' + args[0]) || ''; // for logging
        ...
        .then(function(resp) {
            log.trace('Successfully executed %1 %2', cmd, keyStr);
            return q(resp);
        })
        .timeout(self.timeouts[opType])
        .catch(function(error) {
            log.error('Error executing %1 %2: %3', cmd, keyStr, util.inspect(error));
    ...
    Cache.prototype.add = function(key, val, ttl) {
        ...
        .then(function(resp) {
            log.trace('Successfully executed add on %1', key);
            return q(resp);
        })
        ...
        .catch(function(error) {
            if (error && error.message === 'Item is not stored') {
                ...
                log.error('Error executing add on %1: %3', key, util.inspect(error));
                return q.reject(new Error('Memcache failure'));
            }
        });
    };
