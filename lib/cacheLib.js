(function(){
    ...
    cacheLib.Cache = function(servers, timeouts) {
        var self = this;
        timeouts = timeouts || {};

        // Used within our code to give up on a memcached command and send a response to the client
        self.timeouts = {
            read: timeouts.read || 500,
            stats: timeouts.stats || 2000,
            write: timeouts.write || 2000
        };
    ...
    cacheLib.Cache.prototype.checkConnection = cacheLib._callIfReady(function() {
        return this._memcachedCommand('stats', 'stats').thenResolve(true);
    });
    ...
    // Increments a key from cache
    cacheLib.Cache.prototype.incr = cacheLib._callIfReady(function(key, amount) {
        if(!amount) {
            amount = 1;
        }
        return this._memcachedCommand('incr', 'write', [key, amount]);
    });
    
    // Increments a key from cache and refreshes its ttl
    cacheLib.Cache.prototype.incrTouch = cacheLib._callIfReady(function(key, amount, ttl) {
        var self = this;
        
        ttl = ttl / 1000; // our TTL vals are in ms but memcached expects seconds

        return self.incr(key, amount).then(function(value) {
            return self._memcachedCommand('touch', 'write', [key, ttl]).thenResolve(value);
        });
    });
