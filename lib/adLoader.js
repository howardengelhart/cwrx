...
AdLoader.prototype.fillPlaceholders = function fillPlaceholders(
    ...
    return this.client.getBanners(placeholders.length, placement, ['2x2'], {
        kwlp1: campaign,
        ...
        return q.all(usefulBanners.map(function getCard(banner) {
            ...
            return self.__getCard__(id, params, uuid).then(function addTrackingPixels(card) {
                return addBannerData(banner, card);
            });
...
AdLoader.prototype.loadAds = function loadAds(experience, _categories_, campaignId, uuid) {
    ...
    if (!placement) {
        log.warn('[%1] Experience {%2} has no wildCardPlacement. Aborting.', uuid, experience.id);
        return q.reject(new Error('Experience has no wildCardPlacement.'));
    }
