 ...
 * @param {String} [origin] Origin of the page that the ads will be loaded on.
...
AdLoader.prototype.fillPlaceholders = function fillPlaceholders(
    ...
    return this.__findCards__(
        campaign, params, placeholders.length, origin, uuid
 ...
 *     been loaded.
 */
AdLoader.prototype.loadAds = function loadAds(experience, _categories_, campaign, origin, uuid) {
    var log = logger.getLog();
    ...
    return this.fillPlaceholders(
        experience, categories, campaign, origin, uuid
 ...
 *     found.)
 */
AdLoader.prototype.findCard = function findCard(params, context, origin, uuid) {
    var log = logger.getLog();
    ...
    return this.__findCards__(campaign, context, 1, origin, uuid).then(function getCard(cards) {
        var card = cards[0];
 ...
 */
AdLoader.prototype.getCard = function getCard(id, params, origin, uuid) {
    var log = logger.getLog();
    ...
    return method.call(this, id, params, origin, uuid).tap(function(card) {
        log.trace('[%1] Got card {%2}.', uuid, card.id);
    });
};
