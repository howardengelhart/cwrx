...
AdLoader.prototype.__addTrackingPixels__ = function __addTrackingPixels__(card, meta) {
    ...
    var getPixelUrl = (function() {
        var base = trackingPixel + '?' + querystring.stringify({
        ...
        return function getPixelUrl(event) {
            return base + '&event=' + event + '&d={delay}&cb={cachebreaker}';
        };
    }());

    function dedupe(pixels) {
        return (pixels || []).filter(function(url) {
            return !(/audit(-staging)?\.cinema6\.com/).test(url) &&
                !(/s3\.amazonaws\.com\/c6\.dev\/e2e/).test(url);
        });
    }

    function addPixels(prop, event) {
        campaign[prop] = dedupe(campaign[prop]).concat([getPixelUrl(event)]);
    }
    ...
    Object.keys(card.links || (card.links = {})).forEach(function(type) {
        card.links[type].tracking = dedupe(card.links[type].tracking).concat([
            getPixelUrl('link.' + type)
        ]);
    });
    Object.keys(card.shareLinks || (card.shareLinks = {})).forEach(function(type) {
        card.shareLinks[type].tracking = dedupe(card.shareLinks[type].tracking).concat([
            getPixelUrl('shareLink.' + type)
        ]);
    });
...
AdLoader.prototype.fillPlaceholders = function fillPlaceholders(
    ...
    return this.__findCards__(
        ...
        var cards = arguments[0].filter(function(card) { return cardIds.indexOf(card.id) < 0; })
            .map(function(card) { return self.__addTrackingPixels__(card, pixelData); });
