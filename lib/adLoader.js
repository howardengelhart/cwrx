...
AdLoader.prototype.__addTrackingPixels__ = function __addTrackingPixels__(card, meta) {
    ...
    addPixels('q3Urls', 'q3');
    addPixels('q4Urls', 'q4');
    addPixels('interactionUrls', 'interaction');
...
AdLoader.prototype.pixelFactory = function pixelFactory(card, meta) {
    ...
    return function getPixel(event) {
        if (!self.trackingPixel) {
            return null;
        }

        return base + '&event=' + event + '&sw={screenWidth}&sh={screenHeight}&' +
            'pw={playerWidth}&ph={playerHeight}&' +
            'hitX={screenX}&hitY={screenY}&' +
