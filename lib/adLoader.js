...
AdLoader.prototype.__findCards__ = function __findCards__(campaign, params, amount, uuid) {
    var log = logger.getLog();
    ...
    return q(request.get(url, { qs: qs, json: true }));
};
...
AdLoader.prototype.fillPlaceholders = function fillPlaceholders(
    ...
    return this.__findCards__(
        campaign, params, placeholders.length, uuid
 ...
 *     been loaded.
 */
AdLoader.prototype.loadAds = function loadAds(experience, _categories_, campaign, uuid) {
    var log = logger.getLog();
    ...
    return this.fillPlaceholders(experience, categories, campaign, uuid).tap(function(experience) {
...
AdLoader.prototype.findCard = function findCard(params, context, uuid) {
    ...
    return this.__findCards__(campaign, context, 1, uuid).then(function getCard(cards) {
        var card = cards[0];
 ...
 */
AdLoader.prototype.getCard = function getCard(id, params, uuid) {
    var log = logger.getLog();
    ...
    return method.call(this, id, params, uuid).tap(function(card) {
        log.trace('[%1] Got card {%2}.', uuid, card.id);
    });
};
