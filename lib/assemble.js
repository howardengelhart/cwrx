var include = require('./inject').require,
    path    = include('path'),
    fs      = include('fs-extra'),
    q       = include('q'),
...
function assemble(template,cb){
    ...
    .fail(function(err){
        log.error('[create] [%1] - %2',template.id,err.message);
        assemble.cleanupTmps(log,template,workSpace,gapFileMap);
        throw(err);
    });
}
...
assemble.cleanupTmps = function(log,tmpl,workSpace,gapFileMap){
    if (tmpl.preserve && tmpl.blanks){
        Object.keys(gapFileMap).forEach(function(key){
            log.trace('[create] [%1] - copy %2==>%3',tmpl.id,gapFileMap[key],key);
            try {
                fs.copySync(gapFileMap[key],key);
            }catch(e){
                log.warn('[create] [%1] Error copying blankFile: %2',tmpl.id,e.message);
            }
        });
    }
...
assemble.getSrcInfoID3 = q.fbind(function(log,tmpl,curItem,index){
    if ((curItem.metaData) && (curItem.metaData.duration)){
        log.trace('[create] [%1] getSrcInfoID3: track %2 has duration = %3',
    ...
    log.trace('[create] [%1] getSrcInfoID3 %2',tmpl.id,curItem.src);
...
assemble.getSrcInfo = q.fbind(function(log,tmpl,curItem,index){
    if ((curItem.metaData) && (curItem.metaData.duration)){
        log.trace('[create] [%1] - getSrcInfo %2 - already have it (%3)',
    ...
    log.trace('[create] [%1] - getSrcInfo %2',tmpl.id,curItem.src);
    var deferred = q.defer(),workingItem;
    ffmpeg.probe(curItem.src,function(err,info,cmdline,stderr){
        if (stderr) {
            log.warn('[create] [%1] ffmpeg errors: %2', tmpl.id, stderr.replace('\n','; '));
        }
        if (err) {
            deferred.reject(err);
            return;
...
assemble.calculateGaps = q.fbind(function(log,tmpl,working){
    log.trace('[create] [%1] - calculating gaps',tmpl.id);
...
assemble.createGapFile = q.fbind(function(log,tmpl, workSpace, gapFileMap,curItem, index){
    ...
    if (!curItem.gapBefore) {
        log.trace('[create] [%1] - createGapFile: %2, no gap to create',tmpl.id,index);
    ...
    if (tmpl.blanks){
        ...
        if (fs.existsSync(permFile)){
            log.trace('[create] [%1] - createGapFile: %2, found cached blank: %3',tmpl.id,index,permFile);
    ...
    log.trace('[create] [%1] - make blankfile %2: %3, gap=%4',tmpl.id,index,blankFile,curItem.gapBefore);
    ffmpeg.makeSilentMP3(blankFile,curItem.gapBefore,tmpl,function(err,fpath,cmdline,stderr){
        if (stderr) {
            log.warn('[create] [%1] ffmpeg errors: %2', tmpl.id, stderr.replace('\n','; '));
        }
        if (err) {
            log.error('[create] [%1] - %2, makeSilentMp3 failed: %3',tmpl.id,index,err.message);
            deferred.reject(err);
            return;
        }
        ...
        log.trace('[create] [%1] - resolved create gap file for %2', tmpl.id,index);
...
assemble.concatPlayList = q.fbind(function(log,tmpl,playList){
    ...
    log.trace('[create] [%1] - concat playlist',tmpl.id);
    ffmpeg.concat(playList,tmpl.output,tmpl,function(err,fpath,cmdline,stderr){
        if (stderr) {
            log.warn('[create] [%1] ffmpeg errors: %2', tmpl.id, stderr.replace('\n','; '));
        }
        if (err) {
        ...
        log.trace('[create] [%1] - assemble complete',tmpl.id);
