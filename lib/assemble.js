...
assemble.createGapFile = q.fbind(function(log,tmpl, workSpace, gapFileMap,curItem, index){
    ...
    deferred = q.defer();
    blankFile = path.join(workSpace,uuid().substr(0,10) + '.mp3');
    log.trace('[%1] - make blankfile %2: %3, gap=%4',tmpl.id,index,blankFile,curItem.gapBefore);
    ffmpeg.makeSilentMP3(blankFile,curItem.gapBefore,tmpl,function(err,fpath){
        if (err) {
...
assemble.concatPlayList = q.fbind(function(log,tmpl,playList){
    ...
    ffmpeg.concat(playList,tmpl.output,tmpl,function(err,fpath){
        if (err) {
            deferred.reject(err);
            return;
        }
