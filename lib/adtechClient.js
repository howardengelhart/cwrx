 ...
 *     aborting.
 */
function ADTECHClient(/*config*/) {
    var config = arguments[0] || {};

    this.server = config.server || 'adserver.adtechus.com';
    this.network = config.network || '5473.1';
...
ADTECHClient.__parseBanner__ = (function() {
    ...
    return function __parseBanner__(string) {
        ...
        parts = string.match(FN_REGEX)[1].split(/,\s*/).map(function(part) {
            return part.replace(QUOTE_REGEX, '');
        });
...
ADTECHClient.prototype.__makeURL__ = function __makeURL__(type, placement, params) {
    extend(params, {
        target: '_blank',
        misc: Date.now(),
        cfp: 1
 ...
 */
ADTECHClient.prototype.get = function get(/*...args*/) {
 ...
 */
ADTECHClient.prototype.getBanner = function getBanner(placement, campaignId, bannerId, uuid) {
    ...
    return q(this.get(url)).then(ADTECHClient.__parseBanner__);
};
 ...
 *     * countUrl: ADTECH ad count tracking pixel
 */
ADTECHClient.prototype.getBanners = function getBanners(amount, placement, sizes, keywords, uuid) {
    var log = logger.getLog();
    ...
    return q(this.get(url, { json: true })).then(function parseBanners(response) {
        ...
        return response.ADTECH_MultiAd.map(function parseBanner(banner) {
            try { return ADTECHClient.__parseBanner__(banner.Ad.AdCode); } catch(e) { return null; }
