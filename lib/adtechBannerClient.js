...
function ADTECHBannerClient(/*config*/) {
    ...
    this.keepAlive = ('keepAlive' in config) ? config.keepAlive : true;
 ...
 *     * countUrl: ADTECH ad count tracking pixel
 */
ADTECHBannerClient.prototype.findBanners = function findBanners(
    amount,
    placement,
    sizes,
    keywords,
...
/**
 * Finds a single banner in ADTECH using keywords.
 *
 * @method findBanner
 * @param {Number/String} placement ADTECH placement ID.
 * @param {String} size Banner size to fetch, e.g. "2x2".
 * @param {Object} keywords Map of keywords to send with request.
 * @param {String} [uuid] Contextual UUID (used for logging)
 * @return {Promise} Promise that will be fulfilled with an ADTECH banner containing:
 *     * placementId: ADTECH placement ID
 ...
 *     * countUrl: ADTECH ad count tracking pixel
 */
ADTECHBannerClient.prototype.findBanner = function findBanner(placement, size, keywords, uuid) {
    var log = logger.getLog();
    var url = this.__makeURL__('addyn', placement, extend(keywords, {
        Allowedsizes: size
    }));

    log.trace('[%1] Finding an ADTECH banner: "%2."', uuid, url);

    return this.get(url).then(function parseBanner(banner) {
        try { return ADTECHBannerClient.__parseBanner__(banner); } catch(e) { return null; }
    });
};
