...
function formatContentRange(count, limit, skip) {
    var start = (count !== 0) ? skip + 1 : 0;

    var end = limit ? Math.min(skip + limit, count) : count;

    return 'items ' + start + '-' + end + '/' + count;
}

function setTargetApp(cfg) {
    cfg = cfg || {};
    extend(cfg, {
        hosts: {},
        default: 'selfie'
    ...
    return function targetAppMidware(req, res, next) {
        var host = req.headers.host || '';
        
        for (var key in cfg.hosts) {
            if (!!host.match(key)) {
                req._target = cfg.hosts[key];
                return next();
            }
        }
        req._target = cfg.default;
        return next();
    };
}
...
module.exports.formatContentRange = formatContentRange;
module.exports.setTargetApp = setTargetApp;
