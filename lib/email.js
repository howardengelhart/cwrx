(function(){
    ...
    email.newUpdateRequest = function(sender, recipient, req, campName, reviewLink) { //TODO: tests
        var submitter;

        if (!!req.user) {
            submitter = req.user.company || (req.user.firstName + ' ' + req.user.lastName);
        } else if (req.application) {
            submitter = req.application.appKey;
        }
        
        var data = {
            requester: (req.user && req.user.email) || (req.application && req.application.appKey),
            campName: campName,
