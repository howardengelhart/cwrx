(function(){
    ...
    Model.prototype.personalizeSchema = function(requester) {
        var self = this,
            userSchema = lodash.get(requester, 'fieldValidation.' + self.objName, undefined),
            newSchema = {};
    ...
    Model.checkLimits = function(cfg, value, keyStr) {
        ...
        if ('__length' in cfg && value.length > cfg.__length) {
            var reason = keyStr + ' must have at most ' + cfg.__length +
                         ((typeof value === 'string') ? ' characters' : ' entries');
            return {
                isValid: false,
                reason: reason
            };
     ...
     * - action: 'create' if creating new object, 'edit' if editing existing
    ...
    Model.prototype.validate = function(action, newObj, origObj, requester) {
        var self = this,
            ...
            keyStrPrefix = (self.objName.match(/^[^\.]+\.(.*)*$/) || ['', ''])[1],
            failMsg;
        ...
        function validateField(key, schema, updates, orig, keyStr) {
            ...
            if (schema[key].__required && (updates[key] === undefined || updates[key] === null)) {
                if (orig[key] === undefined || orig[key] === null) {
                    // set field to default if it exists
                    if (schema[key].__default !== undefined) {
                        updates[key] = schema[key].__default;
                    } else {
            ...
            if (updates[key] === undefined) {
                // set default val if defined in schema + not defined on updates
                if (schema[key].__default !== undefined && orig[key] === undefined) {
                    updates[key] = schema[key].__default;
                }
            }
            // trim field if forbidden
            else if (schema[key].__allowed === false) {
            ...
            else if (schema[key].__unchangeable && (orig[key] !== undefined &&
                                                    orig[key] !== null)) {
                log.trace('Key "%1" can only be set once, trimming it off', keyStr);
                replaceField(key, schema, updates, orig);
            }

            // allow null or undefined as a value, and do not continue validating
            ...
            // Do not continue validating if field unchanged; assume existing val is entirely valid
        ...
        var isValid = Object.keys(actingSchema).every(function(key) {
            var keyStr = !!keyStrPrefix ? keyStrPrefix + '.' + key : key;
            return validateField(key, actingSchema, newObj, origObj || {}, keyStr);
        });
     ...
     * req.body, req.origObj, and req.requester, calling next() if isValid === true and done() if
     * isValid === false. The action should be bound in.
     */
    Model.prototype.midWare = function(action, req, next, done) {
        var self = this;

        var validateResp = self.validate(action, req.body, req.origObj || {}, req.requester);
