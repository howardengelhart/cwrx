(function(){
    ...
    Model.prototype.validate = function(action, newObj, origObj, requester) {
        ...
        function validateField(key, schema, updates, orig, keyStr) {
            ...
            // trim field if not allowed for this action //TODO: probs confirm with Howard
            if (schema[key]._actions && schema[key]._actions.indexOf(action) === -1) {
                log.trace('Key "%1" cannot be set/changed on %2', keyStr, action);
                delete updates[key];
                return true;
            }
