(function(){
    ...
    var Journal = function(coll, version, svcName) {
        ...
        hostUtils.getHostname(true).catch(function() {
            return hostUtils.getHostname(); //attempt to get fqdn, if not get regular hostname
        }).then(function(host) {
            self.host = host;
        ...
        q(coll.isCapped()).then(function(result) {
            if (!result) {
    ...
    Journal.prototype.write = function(user, req, data) {
        var log = logger.getLog(),
            ...
            record = {
                user: user,
                application: req.application && req.application.id,
                created: new Date(),
                host: self.host,
                pid: process.pid,
                uuid: req.uuid,
        ...
        return q(self._coll.insertOne(record, { w: 1, j: true }))
        .then(function() {
            log.trace('Successfully wrote event from %1 to %2 journal',
                      user || record.application, self.name);
            return q();
        })
        .catch(function(error) {
            log.warn('Error writing event from %1 to %2 journal: %3',
                     user || record.application, self.name, util.inspect(error));
    ...
    AuditJournal.prototype.middleware = function(req, res, next) {
        var log = logger.getLog(),
            self = this;
        if (!req.requester) {
            log.trace('[%1] No user or app authenticated, so not writing to audit log', req.uuid);
            return next();
        }
        
        self.writeAuditEntry(req, req.user && req.user.id);
