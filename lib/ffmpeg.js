(function(){
    ...
    var exec        = require('child_process').exec,
        ...
        _cmdLineFFmpeg   = 'ffmpeg -loglevel error ',
        _cmdLineFFprobe  = 'ffprobe -loglevel error ',
        _ffmpeg     = {},
        cberr,
        execFFmpeg,
        execFFProbe,
    ...
    createConcatScript = function(fileList,workSpace,cb) {
    ...
    execFFmpeg = function(args,cb){
        var k, cmd = _cmdLineFFmpeg + ' ' + args;
    ...
    execFFProbe = function(args,cb){
        var k,cmd = _cmdLineFFprobe + ' ' + args;
    //    console.log('cmd:' + cmd);
        k = exec(cmd,function(error,stdout,stderr){
    ...
    _ffmpeg.concat = function(){
        ...
        createConcatScript(fileList,options.workspace,function(script){
            execFFmpeg(('-f concat -i ' + script + ' -c:a copy ' + output),
    ...
    _ffmpeg.probe = function(){
        ...
        execFFProbe(('-show_format -i ' + file),function(err,cmdline,output,stderr){
            if (err){
    ...
    _ffmpeg.mergeAudioToVideo = function(){
        ...
        var cmdline = '-i ' + video + ' -i ' + audio +
            ' -strict -2 -filter_complex "[0:1] [1:0] amerge"' +
            ' -filter:a apad -shortest -ac 2 -c:v copy ';
        ...
        cmdline += output;
        execFFmpeg(cmdline, function(err,cmd,stdout,stderr){
                if (err) {
                    if (fs.existsSync(output)){
                        fs.unlinkSync(output);
                    }
    ...
    _ffmpeg.makeSilentMP3 = function(){
        ...
        cmdl += ' ' + outputFile;
        execFFmpeg(cmdl, function(err,cmdline,stdout,stderr){
            if (err) {
                if (fs.existsSync(outputFile)){
