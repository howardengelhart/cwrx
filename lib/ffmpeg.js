...
create_concat_script = function(fileList,workSpace) {
...
_ffmpeg.concat = function(){
    ...
    var script = create_concat_script(fileList,options.workspace);
    exec_ffmpeg(('-f concat -i ' + script + ' -c:a copy ' + output),function(err,cmdline){
//    exec_ffmpeg(('-i \'concat:' + fileList.join('|') + '\' -metadata title="concat" -c copy ' + output), 
//        function(err,cmdline){
            ...
            if (err) {
                ...
                return cberr('ffmpeg err: ' + err,cb,cmdline);
            }
            cb(null,output,cmdline);
        });
};
...
_ffmpeg.probe = function(){
    ...
    exec_ffprobe(('-show_format -i ' + file),function(err,cmdline,output){
        if (err){
            return cberr(err,cb,cmdline);
        }
        ...
        try {
            ...
            cb(null,data,cmdline);
            return;
        }catch(e){
            console.log('EXCEPTION: ' + e); 
        }
        cberr('Unexpected: ' + output,cb,cmdline);
    });
};
...
_ffmpeg.mergeAudioToVideo = function(){
    ...
    var cmdline = '-i ' + video + ' -i ' + audio + 
                  ' -filter_complex "[0:1] [1:0] amerge" -filter:a apad -shortest -c:v copy ';
    ...
    cmdline += output;
    exec_ffmpeg(cmdline, function(err,cmd){
            if (err) {
                if (fs.existsSync(output)){
                    fs.unlinkSync(output);
                }
                return cberr('ffmpeg err: ' + err,cb,cmd);
            }
            cb(null,output,cmd);
        });
};
...
_ffmpeg.makeSilentMP3 = function(){
    ...
    cmdl += ' ' + outputFile;
    exec_ffmpeg(cmdl, function(err,cmdline){
            if (err) {
                if (fs.existsSync(outputFile)){
