(function(){
    ...
    function fromStream(stream) {
        ...
        return new q.Promise(function resolver(resolve, reject) {
            var chunks = [];
            var size = 0;

            stream.on('readable', function getData() {
                var data = this.read();
                if (!Buffer.isBuffer(data)) { objectMode = true; }

                chunks.push(data);
                if (!objectMode) { size += data.length; }
            });
            stream.once('end', function fulfillPromise() {
                if (objectMode) { resolve(chunks); } else { resolve(Buffer.concat(chunks, size)); }
            });
