(function(){
    ...
    campaignUtils.getAccounts = function(advertColl, custColl, req, next, done) {
        var log = logger.getLog(),
            doneCalled = false;
        ...
        function lookup(key, coll) {
            ...
            return q.npost(coll, 'findOne', [{id: String(c6Id)}, {id: 1, adtechId: 1, name: 1}])
            .then(function(obj) {
                if (!obj) {
    ...
    // Formats a click command campaign
    campaignUtils.formatClickCampaign = function(campaign) {
        var campFeatures = {
            clickcommand: true,
            placements: true,
            schedule: true
        ...
        return {
            ...
            placementIdList: campaign.placements, // should be a list of Adtech Placement ids
            pricingConfig: {
                cpm: 0,
                invoiceImpressions: 0
            },
            priority: 3
            // retargeting?
        };
     ...
     * - clickCommand   : true/false, determines which format function to use
     */
    campaignUtils.createCampaign = function(campaign, reqId) {
        var log = logger.getLog(),
            fmt = campaign.clickCommand ? campaignUtils.formatClickCampaign(campaign)
                                        : campaignUtils.formatCampaign(campaign);
        
        return adtech.campaignAdmin.createCampaign(fmt)
        .then(function(resp) {
    ...
    campaignUtils.startCampaigns = function(ids, delay, maxAttempts) {
        var log = logger.getLog();
