(function(){
    ...
    campaignUtils.validateDates = function(obj, delays, reqId) {
        var log = logger.getLog(),
            now = new Date(),
            minStart = new Date(now.valueOf() + (60*60 + 5)*1000),
            minEnd = new Date(now.valueOf() + (60*60 + 10)*1000);
        delays = delays || { start: 0, end: 1 };

        obj.startDate = obj.startDate || new Date(now.valueOf()+ delays.start);
        obj.endDate = obj.endDate || new Date(now.valueOf() + delays.end);
        ...
        obj.startDate = new Date(Math.max(minStart, new Date(obj.startDate))).toISOString();
        obj.endDate = new Date(Math.max(minEnd, new Date(obj.endDate))).toISOString();
     ...
     * - isSponsored    : true/false, determines the priority of the adtech campaign
