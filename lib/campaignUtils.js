(function(){
    ...
    //TODO: comment, test
    campaignUtils.validateDates = function(obj, delays, reqId) {
        var log = logger.getLog();
        delays = delays || { start: 0, end: 0 };

        obj.startDate = obj.startDate || new Date(new Date().valueOf()+ delays.start).toISOString();
        obj.endDate = obj.endDate || new Date(new Date().valueOf() + delays.end).toISOString();
        ...
        if (!(new Date(obj.endDate).valueOf())) {
            log.info('[%1] endDate is not a valid date string: %2', reqId, obj.startDate);
            return false;
        }
    ...
    campaignUtils.formatCampaign = function(campaign) {
        ...
        return {
            ...
            priority: campaign.isSponsored ? 2 : 3,
    ...
    // Format and create a single campaign TODO: update comment with expected fields
    campaignUtils.createCampaign = function(reqId, campaign) {
        var log = logger.getLog();
    ...
    // Edit a campaign, setting TODO: update. keys should be output of makeKeywordLevels
    campaignUtils.editCampaign = function(adtechId, name, startDate, endDate, keys) {
        var log = logger.getLog();
        ...
        .then(function(resp) {
            ...
            record.dateRangeList[0].startDate = startDate || record.dateRangeList[0].startDate;
            record.dateRangeList[0].endDate = endDate || record.dateRangeList[0].endDate;
