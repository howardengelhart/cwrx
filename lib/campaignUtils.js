(function(){
    ...
    campaignUtils.formatCampaign = function(campaign) {
        var startDate = campaign.startDate || new Date(Date.now() + 24*60*60*1000).toISOString(),
            endDate = campaign.endDate || new Date(Date.now() + 366*24*60*60*1000).toISOString();
        
        if (new Date(endDate) < new Date(startDate)) {
            endDate = new Date(new Date(startDate).valueOf() + 24*60*60*1000).toISOString();
        }
    ...
    campaignUtils.editCampaign = function(name, campaign, keys, reqId) {
        ...
        .then(function(resp) {
            ...
            if (resp.statusTypeId === kCamp.STATUS_ACTIVE) {
                ...
                startDate = record.dateRangeList[0].startDate;
            } else {
                startDate = campaign.startDate || record.dateRangeList[0].startDate;
            }
            
            endDate = campaign.endDate || record.dateRangeList[0].endDate;

            if (new Date(endDate) < new Date(startDate)) {
                endDate = new Date(new Date(startDate).valueOf() + 24*60*60*1000).toISOString();
            }
            
            record.dateRangeList[0].startDate = startDate;
            record.dateRangeList[0].endDate = endDate;
