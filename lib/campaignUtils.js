(function(){
    ...
    campaignUtils.statusCheck = function(permitted, req, next, done) {//TODO: comment, test, rename?
        var log = logger.getLog();
    ...
    campaignUtils.startCampaigns = function(ids, delay, maxAttempts) { //TODO: test, comment
        var log = logger.getLog();
        ...
        return q.all(ids.map(function(id) { // start each campaign
        ...
        .then(function() {
            ...
            ids.forEach(function(id) { // set up list of deferreds
                deferreds[id] = q.defer();
                promises.push(deferreds[id].promise);
            });

            // This eventually resolves or rejects all promises in deferreds
            campaignUtils.pollStatuses(deferreds, kCamp.STATUS_ACTIVE, delay, maxAttempts);

            return q.all(promises);
        })
        .catch(function(error) {
            log.error('Error starting campaigns %1: %2', ids, error);
            return q.reject(new Error('Adtech failure'));
        });
    };

    campaignUtils.holdCampaigns = function(ids, delay, maxAttempts) { //TODO: test, comment
        var log = logger.getLog();
        ...
        .then(function() {
            ...
            // This eventually resolves or rejects all promises in deferreds
            campaignUtils.pollStatuses(deferreds, kCamp.STATUS_HOLD, delay, maxAttempts);

            return q.all(promises);
        })
        .catch(function(error) {
            log.error('Error holding campaigns %1: %2', ids, error);
            return q.reject(new Error('Adtech failure'));
        });
    };

    //TODO: combine with deleteCampaign?  Simplify/modularize all of this?
    campaignUtils.stopCampaigns = function(ids, delay, maxAttempts) { //TODO: test, comment
        var log = logger.getLog();
        ...
        .catch(function(error) {
            log.error('Error stopping campaigns %1: %2', ids, error);
            return q.reject(new Error('Adtech failure'));
        });
    };
