(function(){
    ...
    //TODO: test following four functions
    
    campaignUtils.validateCardDates = function(body, origObj, requester, delays, reqId) {
        if (!body.cards) {
            return { isValid: true };
        }
    ...
    campaignUtils.ensureUniqueIds = function(body) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (!objUtils.isListDistinct(ids)) {
                return { isValid: false, msg: keys[i] + ' must be distinct' };
            }
        }
        return { isValid: true };
    };
    ...
    campaignUtils.ensureUniqueNames = function(body) {
        ...
        for (var i = 0; i < body.cards.length; i++) {
            ...
            if (names.indexOf(name) !== -1) {
                return {
                    isValid: false,
                    msg: 'cards[' + i + '] has a non-unique name: "' + name + '"'
                };
            } else {
                names.push(name);
            }
    ...
    campaignUtils.computeCost = function() {
        //TODO: replace this with actual values/logic!
        return 0.09;
    };
    
    campaignUtils.validatePricing = function(body, origObj, requester, model) {
        var origPricing = origObj && origObj.pricing,
        ...
        if (actingSchema.pricing.cost.__allowed === true) {
            body.pricing.cost = body.pricing.cost || origPricing && origPricing.cost ||
                                campaignUtils.computeCost();
        }
        else { // otherwise recompute the cost each time
            body.pricing.cost = campaignUtils.computeCost();
        }
        ...
        if (ratio < limitMin || ratio > limitMax) {
            return {
                isValid: false,
                msg: 'dailyLimit must be between ' + limitMin + ' and ' + limitMax +
