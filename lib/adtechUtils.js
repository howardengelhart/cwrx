(function(){
    ...
    adtechUtils.lookupKeywords = function(ids) {
        var log = logger.getLog(),
            keys = Object.keys(keywordCache);
        
        return q.all(ids.map(function(id) {
            for (var i = 0; i < keys.length; i++) {
                if (keywordCache[keys[i]] === id) {
                    return q(keys[i]);
                }
            }
            ...
            .then(function(keyword) {
                ...
                keywordCache[keyword] = id;
                setTimeout(function() {
                    delete keywordCache[keyword];
                }, keywordTTL);
                return q(keyword);
            })
            .catch(function(error) { //TODO: catch a 404 error here?
    ...
    adtechUtils.makeKeywords = function(keywords) {
        var log = logger.getLog();
        
        return q.all(keywords.map(function(keyword) {
            if (keywordCache[keyword]) {
                return q(keywordCache[keyword]);
            }
    ...
    adtechUtils.formatCampaign = function(campaign, keywords) {
    ...
    adtechUtils.createBanners = function(bannCfgs, oldBanns, type, campId) {
        var log = logger.getLog(),
            toCreate = [];
        oldBanns = oldBanns || [];
        ...
        bannCfgs.forEach(function(item, idx) {
            ...
            if (!existing) { // Create new banners for anything not in oldBanns
                toCreate.push(adtechUtils.formatBanner(type, item.id));
    ...
    adtechUtils.cleanBanners = function(newBanners, oldBanners, campId) {
        var log = logger.getLog();
