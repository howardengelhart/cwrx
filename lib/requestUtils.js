(function(){
    ...
    // Send a signed request, calling signReq on opts first and then requestUtils.qRequest
    requestUtils.makeSignedRequest = function(creds, method, opts, files, jobPolling) {
        signatures.setAuthHeaders(creds, method, opts);
     ...
     */
    requestUtils.proxyRequest = function(req, method, opts, files, jobPolling) {
        ...
        return requestUtils.makeSignedRequest(creds, method, opts, files, jobPolling);
    };

    // Check if can connect to <port> on <host>; times out after <timeout> ms
