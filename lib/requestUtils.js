(function(){
    ...
    requestUtils.portScan = function(host, port, timeout) {
        /* TODO: should we bother checking if nc is installed?
        return q.npost(cp, 'exec', ['which nc'])
        .spread(function(stdout, stderr) {
            if (!stdout.match(/nc/)) {
                return q.reject('No nc found');
            }
        })
        ...
        */
        ...
        var cmd = ['nc', '-zv', host, port, '2>&1'].join(' '),
            opts = { timeout: timeout || 2000 };

        return q.npost(cp, 'exec', [cmd, opts])
        .spread(function(stdout/*, stderr*/) {
            if (!stdout.match(/succeeded/)) {
                return q.reject('Connection refused');
            }
            return true;
        });
    };
