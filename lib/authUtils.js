(function(){
    ...
    /**
     * Checks that the requester is authorized to perform the action, based on the opts.
     * If opts.permissions is set, will check that requester has each obj-verb defined in their
     * permissions.
     * If opts.entitlements is set, will check that requester has same val for each entitlement.
     */
    authUtils.authorize = function(opts, requester) {
        var validPerms = true,
            validEnts = true;
            
        if (!!opts.permissions && Object.keys(opts.permissions).length > 0) {
            var cfgPerms = opts.permissions,
                reqPerms = requester.permissions;

            validPerms = !!reqPerms && Object.keys(cfgPerms).every(function(key) {
                return !!reqPerms[key] && !!reqPerms[key][cfgPerms[key]];
            });
        }

        if (!!opts.entitlements && Object.keys(opts.entitlements).length > 0) {
            var cfgEnts = opts.entitlements,
                reqEnts = requester.entitlements;

            validEnts = !!reqEnts && Object.keys(cfgEnts).every(function(key) {
                return reqEnts[key] === cfgEnts[key];
            });
        }
        
        return validPerms && validEnts;
    };
     ...
     * }
     */
    authUtils.authenticateUser = function(req, statuses) {
     ...
     * }
     */
    authUtils.authenticateApp = function(req, tsGracePeriod) {
        var log = logger.getLog(),
    ...
    authUtils.middlewarify = function(opts) {
        ...
        return function authenticate(req, res, next) {
            return q.all([
                authUtils.authenticateUser(req, opts.userStatuses),
                !!opts.allowApps ? authUtils.authenticateApp(req, opts.tsGracePeriod)
                                 : q({ success: false })
            ])
            .spread(function(userResult, appResult) {
                ...
                // Check if combined requester has privileges necessary for endpoint
                if (!authUtils.authorize(opts, req.requester)) {
                    log.info('[%1] Unauthorized: %2 has insufficient privileges',
