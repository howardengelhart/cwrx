(function(){
    ...
    authUtils.createRequester = function(req) { //TODO: comment, test
        var user = req.user || {},
            app = req.application || {},
            requester = { id: user.id || app.id };

        //TODO: include any other info on requester? some flag to note it came from user + app?
        ...
        requester.applications = authUtils.mergeApplications([ //TODO: ajfwfjoaisdufoiaudf
     ...
     * properties. When passed a status param, will check that the user has that value
     * as its status.
     */
    authUtils.authUser = function(req, statuses) {
        //TODO: combine with authUtils.getUser, probs
    ...
    authUtils.middlewarify = function(opts) {
        ...
        if (opts.allowApps) {
            sigVerifier = new signatures.Verifier(authUtils._db, opts.tsGracePeriod);
        }
        
        return function authorize(req, res, next) {
            return q([
                authUtils.authUser(req, opts.userStatuses),
                !!sigVerifier ? sigVerifier.verifyReq(req) : q({ success: false })
            ])
            .spread(function(userResult, appResult) {
                if (userResult.success && userResult.user) {
                    req.user = userResult.user;
                } else if (userResult.code) {
    ...
    authUtils.objMidware = function(objName, opts) { //TODO: rename, rethink?
        var midWareObj = {};
