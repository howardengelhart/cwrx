(function(){
     ...
     * as its status.
     */
    authUtils.authUser = function(id, perms, statuses) {
        return authUtils.getUser(id).then(function(user) {
            ...
            var hasPermittedStatus = false;
            for(var i=0;i<statuses.length;i++) {
                if(user.status === statuses[i]) {
                    hasPermittedStatus = true;
                    break;
                }
            }
            if (!hasPermittedStatus) {
                ...
                if(statuses.length === 1) {
                    statusesText = statuses[0];
                } else {
                    statusesText = statuses.slice(0, statuses.length - 1).join(', ');
                    statusesText = statusesText + ', or ' + statuses[statuses.length - 1];
                }
                return q('User is not ' + statusesText);
     ...
     */
    authUtils.middlewarify = function(perms, db, userStatuses) {
        ...
        return function authorize(req, res, next) {
            ...
            authUtils.authUser(req.session.user, perms, userStatuses).then(function(result) {
                if (result.user) {
                    req.user = result.user;
                    log.trace('[%1] User %2 is authorized to %3',req.uuid,req.user.id,routeStr);
                    next();
                } else {
