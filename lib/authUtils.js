(function(){
     ...
     * as its status.
     */
    authUtils.authUser = function(id, perms, status) {
        return authUtils.getUser(id).then(function(user) {
            if (!user) {
                return q('User not found');
            }
            var requiredStatus = status || Status.Active;
            if (user.status !== requiredStatus) {
                return q('User is not ' + requiredStatus);
     ...
     */
    authUtils.middlewarify = function(perms, db, status) {
        ...
        return function authorize(req, res, next) {
            ...
            authUtils.authUser(req.session.user, perms, status).then(function(result) {
                if (result.user) {
