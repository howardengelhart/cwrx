(function(){
    ...
    module.exports = function(freshTTL, maxTTL, collection) {
        ...
        /**
         * Return a function that validates a username/password combination that can be used as
        ...
        authUtils.userPassChecker = function(coll) {
            ...
            return function authenticate(req, res, next) {
                ...
                if (!req.body || !req.body.username || !req.body.password) {
                    log.info('[%1] Unauthorized request %2: Need username and password',
                             req.uuid, routeStr);
                    return res.send(400, 'Must provide username and password');
                }
                q.npost(coll, 'findOne', [{username: req.body.username}])
                .then(function(account) {
                    if (!account) {
                        log.info('[%1] Unauthorized request %2: unknown username %3',
                                 req.uuid, routeStr, req.body.username);
                    ...
                    log.error('[%1] Error checking user/pass for %2 for %3: %4',
                              req.uuid, req.body.username, routeStr, error);
                    res.send(500, 'Error checking authorization of user');
                });
            };
        };
