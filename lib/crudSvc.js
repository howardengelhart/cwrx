(function(){
     ...
     * config should be the service's config, including the api section.
     */
    CrudSvc.fetchRelatedEntity = function(opts, config, req, next, done) {
        if (!opts || !opts.idPath || !opts.objName) {
            throw new Error('Must provide opts.idPath and opts.objName');
        }
        
        config.api[opts.objName].baseUrl = config.api[opts.objName].baseUrl || urlUtils.resolve(
            config.api.root,
            config.api[opts.objName].endpoint
        );
        ...
        return requestUtils.proxyRequest(req, 'get', {
            url: urlUtils.resolve(config.api[opts.objName].baseUrl, id)
        })
        .then(function(resp) {
