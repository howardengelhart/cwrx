(function(){
     ...
     *  - parentOfUser: if true, the 'own' scope will allow actions on an object if the requester
     *      belongs to the object, rather than if the requester owns the object
     */
    function CrudSvc(coll, prefix, opts, schema) {
        ...
        self._parentOfUser = opts.parentOfUser !== undefined ? opts.parentOfUser : false;
    ...
    CrudSvc.prototype.checkScope = function(user, obj, verb) {
        ...
        function matchesUser() {
            return !!self._parentOfUser ? (user[self.singularizeName()] === obj.id)
                                        : (user.id === obj.user);
        }
        function matchesOrg() {
    ...
    CrudSvc.prototype.userPermQuery = function(query, requester) {
        ...
        if (readScope === Scope.Own || readScope === Scope.Org) {
            if (!!self._parentOfUser) {
                orClause = { $or: [ { id: requester[self.singularizeName()] } ] };
            } else {
