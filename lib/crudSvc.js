(function(){
     ...
     */
    function CrudSvc(coll, prefix, opts, cache) {
        opts = opts || {};
        var self = this;
        self._coll = coll;
        self._prefix = prefix;
        ...
        self.reqTimeouts = {
            enabled: (opts.enableReqTimeouts && !!cache) || false, // default == turned off for now
            timeout: opts.reqTimeout || 5*1000,
            cacheTTL: opts.cacheTTL || 24*60*60*1000, //TODO: rethink?
        };
        
        //TODO: these are for testing, remove
        self.reqTimeouts.enabled = opts.enableReqTimeouts || true;
        self.reqTimeouts.timeout = 2000;
        self.reqTimeouts.cacheTTL = 20*1000;
    ...
    //TODO: should these functions go elsewhere, or at least not on the prototype, if we want to use with non-CrudSvc services?
    //TODO: comment, test
    CrudSvc.prototype.setReqTimeout = function(req, res) {
        var self = this,
            log = logger.getLog(),
            timeoutObj = { timedOut: false };
            
        if (!self.reqTimeouts.enabled) {
            return timeoutObj;
        }
        ...
        timeoutObj.timeout = setTimeout(function() {
            ...
            var data = { code: 202, body: { reqId: req.uuid } };

            self.cache.add('req:' + req.uuid, data, self.reqTimeouts.cacheTTL)
            .then(function() {
                log.info('[%1] Successfully wrote 202 to cache', req.uuid);
                res.send(202, { reqId: req.uuid });
            })
            .catch(function(error) {
                log.error('[%1] Failed to write 202 to cache: %2',
                          req.uuid, (error && error.stack || error));
                /* TODO:
                    In this case, not sending 202 here would mean client would keep waiting for response
                    until main function returned. Maybe a decent way to handle this error case?
                */
            });
        }, self.reqTimeouts.timeout);
        
        return timeoutObj;
    };
    
    //TODO: comment, test
    CrudSvc.prototype.checkReqTimeout = function(req, result, timeoutObj) {
        var self = this,
            log = logger.getLog(),
            body;
        ...
        if (!timeoutObj.timedOut) {
            ...
            return; //TODO: should we move normal res.send() in here?
        }
        
        if (result.state === 'fulfilled') {
            body = { code: result.value.code, body: result.value.body };
        } else {
            body = { code: 500, body: 'Internal Error' };
        }

        return self.cache.set(
            'req:' + req.uuid,
            body,
            self.reqTimeouts.cacheTTL
        )
        .then(function() {
        ...
        .catch(function(error) {
            log.error('[%1] Failed to write final response to cache: %2',
                      req.uuid, (error && error.stack || error));
            /* TODO:
                Client might be stuck with 202 forever. Are they SOL?  Should we retry?
            */
        });
    };
     ...
     */
    CrudSvc.prototype.getObjs = function(query, req, res, multiGet) {
        var self = this,
        ...
        var timeoutObj = self.setReqTimeout(req, res);
        ...
        return deferred.promise.finally(function() {
            return self.checkReqTimeout(req, deferred.promise.inspect(), timeoutObj);
        });
    };
     ...
     * middleware.
     */
    CrudSvc.prototype.createObj = function(req, res) {
        var self = this,
            deferred = q.defer(),
            log = logger.getLog();
     ...
     * as middleware.
     */
    CrudSvc.prototype.editObj = function(req, res) {
        var self = this,
            deferred = q.defer(),
            log = logger.getLog();
    ...
    CrudSvc.prototype.deleteObj = function(req, res) {
        var self = this,
            deferred = q.defer(),
            log = logger.getLog();
