(function(){
    ...
    CrudSvc.prototype.checkScope = function(req, obj, verb) {
        ...
        function matchUser() {
            return req.user && !!self._ownedByUser ? (req.user.id === obj.user)
                                                   : (req.user[self.singularizeName()] === obj.id);
        }
        function matchOrg() {
    ...
    CrudSvc.prototype.userPermQuery = function(query, req) {
        var self = this,
            ...
            readScope = (req.requester.permissions[self.objName] || {}).read,
            log = logger.getLog();
        ...
        if (readScope === Scope.Own || readScope === Scope.Org) {
            if (!!self._ownedByUser) {
                ...
                var parentId = (req.user && req.user[self.singularizeName()]);
    ...
    CrudSvc.prototype.getObjs = function(query, req, multiGet) {
        ...
        self.runMiddleware(req, 'read', function() {
            ...
            log.info('[%1] Requester %2 getting %3 with query %4, opts: %5',
                     req.uuid, req.requester.id, self.objName, JSON.stringify(req._query),
                     JSON.stringify(opts));
