...
//TODO: come up with better names for the module + its classes
    ...
    appAuthUtils.formatEndpoint = function(method, url) {
    ...
    appAuthUtils.Authenticator.prototype.setHeaders = function(method, reqOpts) {
        ...
        var qs = reqOpts.query ? querystring.stringify(reqOpts.query)
                               : urlUtils.parse(reqOpts.url).query;
            
        var data = {
            appKey      : self.appKey,
            bodyHash    : bodyHash,
            endpoint    : appAuthUtils.formatEndpoint(method, reqOpts.url),
            nonce       : nonce,
            qs          : qs,
            timestamp   : ts
        };
    ...
    appAuthUtils.Verifier.prototype.middlewarify = function(required) {
        var self = this,
            log = logger.getLog(),
        ...
        return function authorize(req, res, next) {
            ...
            .then(function(app) {
                ...
                var signData = {
                    appKey      : appKey,
                    bodyHash    : bodyHash,
                    endpoint    : appAuthUtils.formatEndpoint(req.method, req.originalUrl),
                    nonce       : nonce,
