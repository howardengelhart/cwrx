var __ut__ = ((module.parent) && (module.parent.filename) && 
             (module.parent.filename.match(/logger.spec.js$/))),
    __path = require('path'),
...
FileLogMedia.prototype.rotateLogs = function(){
    var self = this;
    if (self.backupLogs === 0) {
        __fs.removeSync(self.logBasePath);
        return;
    }

    var files     = __fs.readdirSync(self.logDir),
...
function Logger(name,opts) {
    ...
    if ((opts.logLevel !== undefined) && (opts.logLevel !== null)) {
        self.setLogLevel(opts.logLevel);     
    }
...
exports.createLogger = function(cfg,name) {
    if (!logInstances){
        process.on('exit',function(){
            Object.keys(logInstances).forEach(function(instance){
                logInstances[instance].commit();
            });
        });
        logInstances = {};
    }
...
exports.getLogger = function(name){
    if (!name) {
        ...
        return exports.createLogger(undefined,'default');
    }
